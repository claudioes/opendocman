...
<form name="author_note_form" method="POST" action="toBePublished.php{if $mode eq 'root'}?mode=root{/if}">

    <table name="author_note_table">
        <tr>
            <td>{$g_lang_email_to}</td>
            <td><input type="text" name="to" value="Author(s)" size="15" {$access_mode|escape:'html'}></td>
		</tr>
        <tr>
            <td>{$g_lang_email_subject}</td>
            <td><input type="text" name="subject" size=50 value="" size="30" {$access_mode|escape:'html'}></td>
        </tr>
        <tr>
