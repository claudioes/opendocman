...
if ($reviewIdCount > 0) {
    echo '<div style="margin: 20px 0;"><img src="images/exclamation.gif" style="margin-right: 10px;" /> <a href="toBePublished.php?state=1">'.msg('message_documents_waiting'). '</a>: ' . e::h($reviewIdCount)  . '</a></div>';
}

$rejected_files_obj = $user_obj->getRejectedFileIds();

if (isset($rejected_files_obj[0]) && $rejected_files_obj[0] != null) {
    echo '<div style="margin: 20px 0;"><img src="images/exclamation_red.gif" style="margin-right: 10px;" /> <a href="rejects.php?state=1">'. msg('message_documents_rejected') . '</a>: ' .sizeof($rejected_files_obj) . '</div>';
}

$llen = $user_obj->getNumExpiredFiles();
if ($llen > 0) {
    echo '<div style="margin: 20px 0;"><img src="images/exclamation_red.gif" style="margin-right: 10px;" /> <a href="javascript:window.location=\'search.php?submit=1&author_locked_files=-1&sort_by=id&sort_order=asc\'">' .msg('message_documents_expired'). ': ' . e::h($llen) . '</a></div>';
}
